...
const {Wheat} = require('./crops.js');
const {Sugarcane} = require('./crops.js');
const {Pig} = require('./animals.js');
const {Cow} = require('./animals.js');
const {Horse} = require('./animals.js');
...
//test3 - tests this.crops array
test('We can add a crop to our farm', () => {
  const farm = new Farm('TEST_NAME')
  farm.addCrop(new Wheat(100))
})

//test4 - testing the calculateIncome function
...
//test5 - this.animal array
test('We can add an animal to our farm', () => {
  const farm = new Farm('TEST_NAME')
  farm.addAnimal(new Pig(13))
})

//test 6 - checking income calculateIncomeAnimalstest('Animals can be added to the farm', () => {
...
test('Animals will be taken into account for calculateIncome', () => {
  const farm = new Farm()
  farm.addAnimal(new Cow(100))
  expect(farm.calculateIncomeAnimals()).not.toBe(0)
})
